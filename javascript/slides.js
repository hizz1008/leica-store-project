...
const slideProductUl = document.querySelector(".slide-product_slides_container")
const slideProductLi = document.querySelectorAll(".slide-product_slides_container li")
...
function nextMove() {
  ...
  console.log(slideProductCurrentIdx,slideProductCount)
}
...
function prevMove() {
  ...
  if (slideProductCurrentIdx >= -slideProductCount+4) {
    ...
    const offset = 660 * (slideProductCurrentIdx);
    console.log(slideProductCurrentIdx)
    slideProductUl.style.left = `${-offset}px`;
    //슬라이드 컨테이너의 left값이 100단위로 -가 될 수 있게 작성
  } else {
    setTimeoutAnimatedClass();
...
function updateWidth(){
  slideProductUl.style.width = (slideProductWidth + slideProductMargin)*slideProductCount - slideProductMargin + "px";
  const slideProductLi = document.querySelectorAll(".slide-product_slides_container li")
  let newslideCount = slideProductLi.length;

  let newWidth = (slideProductWidth+slideProductMargin)*newslideCount - slideProductMargin + "px";

  slideProductUl.style.width = newWidth;
}
function setInitialPos(){
  const initialTranslateValue = -(slideProductWidth+slideProductMargin)*slideProductLi.length

  slideProductUl.style.transform = 'translateX('+initialTranslateValue+'px)';
}
